...
function lib_ttn.register_titan(entity)
  ...
  elseif titan_type.class == shared.class_warbringer then
    titan_info.guns = {
      ...
      -- lib_ttn.init_gun(shared.weapon_artillery),
  ...
  titan_info.guns = table.slice(titan_info.guns, 1, #titan_type.mounts)
...
local function titan_death(titan_info)
  ...
  local target
  local scatter = 1 + titan_info.class /3
  ...
  for i = 0, explo_count do
    ...
    titan_info.surface.create_entity{
      ...
      position=source, source=source, target=target, speed=i+1,
    }
  end
  ...
  for k, cannon in pairs(titan_info.guns) do
    ...
    if weapon_type.ammo and cannon.ammo_count > 0 then
...
function lib_ttn.titan_removed(titan_info, is_death)
  ...
  for wi, cannon in ipairs(titan_info.guns) do
    if cannon.entity then
