...
local function on_post_warp(event)
	-- event = {newsurface=f, newplanet=w}
	for k, obj in pairs(game.players) do
		remote.call("space-exploration", "lock_player_respawn_location", {
			player = obj,
			position = {x=0, y=0},
			surface_name = event.newsurface.name,
		})
	end
end
...
remote.add_interface("WSEK", {
	...
	warptorio2_provide_planet = function(args)
		...
		-- TODO: make orbits more often, to prevent land structure destruction
		local zone
...
lib:on_load(function()
	script.on_event(remote.call("warptorio", "get_event", "on_post_warp"), on_post_warp)
end)
