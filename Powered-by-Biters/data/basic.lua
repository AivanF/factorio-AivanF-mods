...
-- Naively only checks the first tier of effects, but it works for vanilla
-- and should work for any modded items that aren't super fancy, which we should probably leave alone anyway
...
make_potion("raw-meat", mod_path .. "/graphics/raw.png",			prettynum(15),	prettynum(12),	2,	-0.15, 1.5)	--	-.15 move/use, (135 on use); 50//s+30/s=80/s		(-.3 = -2.7 tiles)
make_potion("roast-meat", mod_path .. "/graphics/roast.png",		prettynum(12),	prettynum(30),	1.5,0.10)		--	0.00 move/use, (210 on use); 40//s + 100/s= 140/s	(.15 = 1.2 tiles)	
make_potion("steamed-bun", mod_path .. "/graphics/steamed-bun.png",		prettynum(20),	prettynum(60),	3,	0.25, 2)	--	move/use, ( on use); 50//s + 100/s=  150/s			(.25 = (2 tiles/s) )
...
to_oil.ingredients = {{"raw-meat", 5}}
to_oil.energy_required = 2
to_oil.results = {{
	type = "fluid",
	name = "crude-oil",
